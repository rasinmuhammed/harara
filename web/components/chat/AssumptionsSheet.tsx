      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed right-0 top-0 z-50 h-full w-full max-w-md overflow-y-auto border-l border-border bg-bg-raised p-5 shadow-1">
          ...
          <ul className="flex list-disc flex-col gap-3 pl-4 text-sm leading-relaxed text-ink-secondary">
            ...
            <li><strong className="text-ink">The 32.1 C line is drawn, not enforced.</strong> The plan minimises retained heat load at equal output within an on-site window. Hours where it still schedules work above 32.1 C are flagged. Decision 17/2021 requires the hard stop on top.</li>
            <li><strong className="text-ink">Worker time is a constraint.</strong> The plan holds the crew on site no longer than the fixed 10:00 to 15:30 rule keeps them, and in no more work blocks. Time on site and total heat dose are reported next to the peak so the trade is visible.</li>
            ...
            <li><strong className="text-ink">Evidence.</strong> Over 236 held-out days, the peak-load advantage the earlier optimiser showed does not survive once the day is held short: it came from spreading work across a longer on-site day. A plain earlier start is better on heat but leaves work undelivered on about half of peak-season days. See the <a className="underline" href="https://github.com/rasinmuhammed/harara/blob/main/docs/technical_report.md" target="_blank" rel="noreferrer">technical report</a> and <a className="underline" href="https://github.com/rasinmuhammed/harara/blob/main/docs/results_ledger.md" target="_blank" rel="noreferrer">results ledger</a>.</li>
          </ul>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
