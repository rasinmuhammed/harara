...
function Cell({ label, value, digits = 2, unit, sub, delta }: {
  label: string; value: number; digits?: number; unit?: string; sub?: string; delta?: string;
}) {
...
export function ComparisonStrip({ s, showReactive }: { s: PlanSummary; showReactive?: boolean }) {
  return (
    <div role="group" aria-label="Plan versus the other rules" className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
      <Cell label="Retained load, peak" value={s.peak_plan}
        sub={`ban ${fmt(s.peak_calendar)}${showReactive ? ` · hot-stop ${fmt(s.peak_reactive)}` : ""}`.replace("·", "·")}
        delta={`${s.pct_peak_reduction > 0 ? "−" : "+"}${fmt(Math.abs(s.pct_peak_reduction), 1)}% vs ban`.replace("−", "−")} />
      <Cell label="Tail, p90" value={s.tail_plan} sub={`ban ${fmt(s.tail_calendar)}`}
        delta={`${s.pct_tail_reduction > 0 ? "−" : "+"}${fmt(Math.abs(s.pct_tail_reduction), 1)}% vs ban`} />
      <Cell label="Hours delivered" value={s.work_hours_delivered_plan} digits={1} unit="h"
        sub={`ban ${fmt(s.work_hours_delivered_calendar, 1)} h`} />
